...
variable "gaia_ami_id" {
  description = <<-EOT
    Check Point Gaia AMI ID. Generated by scripts/import-gaia-ami.sh.
    Set via environment variable before running terraform apply:
      export TF_VAR_gaia_ami_id=$(aws ssm get-parameter \
        --name /checkpoint-demo/gaia-ami-id --with-decryption \
        --query 'Parameter.Value' --output text)
    Check Point instances are skipped when this is empty.
  EOT
  type        = string
  default     = ""

  validation {
    condition     = var.gaia_ami_id == "" || can(regex("^ami-", var.gaia_ami_id))
    error_message = "gaia_ami_id must be a valid AMI ID (ami-xxxxxxxxxxxxxxxxx) or left empty."
  }
}
